(function () {
  ...
  const DEMO = [
    ...
    { id:'c5', name:'Jian', type:'Cat', breed:'Puspin', age:3, gender:'Male', bio:'Loves to sleep and cuddle with blankets.', img:'images/jian meow.jpg' },
  ...
  /* ⭐⭐⭐ BREED SYSTEM INSERTED (NO REMOVALS) ⭐⭐⭐ */
  ...
  function getUserPets() {
    return JSON.parse(localStorage.getItem("pets") || "[]").map(p => ({
      ...p,
      img: p.image || p.img || "",
      gender: p.gender || "Unknown"
    }));
  ...
  /* ⭐⭐⭐ FILTER FUNCTION WITH BREED ADDED ⭐⭐⭐ */

  function applyFilters() {
    let list = getPets();

    const qtype = $('filterType').value;
    const qbreed = $('filterBreed').value;   // ⭐ ADDED
  ...
  /* ⭐ MODALS + ADOPTION LOGIC (UNCHANGED) ⭐ */
  ...
  $("openAddPetFromInfo").addEventListener("click", () => {
    ...
    let list = JSON.parse(localStorage.getItem("adoptionList") || "[]");
